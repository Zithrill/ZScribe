...
def get_git_config_model(hook_type=None):
    try:
        if hook_type == 'commit':
            return subprocess.check_output(['git', 'config', 'zscribe.model.commit']).decode('utf-8').strip()
        elif hook_type == 'pr':
            return subprocess.check_output(['git', 'config', 'zscribe.model.pr']).decode('utf-8').strip()
        else:
